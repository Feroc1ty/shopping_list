package ru.rykunov.shopping_list.billing
            ...
            if(!purchase.isAcknowledged){
                ...
                bClient?.acknowledgePurchase(acParams){
                    if (it.responseCode == BillingClient.BillingResponseCode.OK) {
                        savePref(true)
                        Toast.makeText(activity, "Ads disabled!", Toast.LENGTH_LONG).show()
                    } else{
                        savePref(false)
                        Toast.makeText(activity, "Failed to make a purchase", Toast.LENGTH_LONG).show()
                    }
                }
            }
        }
